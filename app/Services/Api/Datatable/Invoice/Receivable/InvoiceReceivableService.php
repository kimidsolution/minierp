...
namespace App\Services\Api\Datatable\Invoice\Receivable;
...
class InvoiceReceivableService
{
    ...
    {
        $get_data = Invoice::with(['partner'])
            ->where('type', Invoice::TYPE_RECEIVABLE);
        ...
        return Datatables::of($invoice)
            ...
            ->addColumn('total_amount', function ($invoice) {
                return view('datatable.invoice.receivable.variable', compact('invoice'));
            })
            ->addColumn('action', function ($invoice) {
                return view('datatable.invoice.receivable.link-action', compact('invoice'));
            })

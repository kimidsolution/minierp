...
namespace App\Services\Api\Datatable\Voucher\Receivable;
...
class VoucherReceivableService
{
    public function handle(Request $request)
    {
        if ('false' !=  $request->is_active) {
            $getData = Voucher::where('type', Voucher::TYPE_RECEIVABLE);
        } else {
            $getData = Voucher::withTrashed()->where('type', Voucher::TYPE_RECEIVABLE);
        }
        ...
        return Datatables::of($vouchers)
            ...
            ->addColumn('action', function ($vouchers) {
                return view('datatable.voucher.receivable.link-action', compact('voucher'));
            })

...
$(function () {
    ...
    getListTypeInvoice(companyIdDefault);
...
$(document).on('change', '.invoice', function(e){
    ...
    fetchDataInvoice(e, routeFetchInvoice, dataSend, indexForm);
...
function fetchDataPartner(userid, type, companyId) {
...
function fetchDataInvoiceList(routeFetchInvoice, dataSend, counter) {
    let dataArray = [];
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status) {
                ...
                $("#noinvoice-" + counter).append('<option selected="selected" value="">Select Invoice...</option>');

                $.each(response.data,function(key, value) {
                    if (jQuery.inArray(value.id.toString(),dataArray) == -1) {
                        $("#noinvoice-" + counter).append('<option value=' + value.id + '>' + value.text + '</option>');
                    }
                });
            }
        }
...
function setListInvoice(counter) {
    let idPartner = $("#pelanggan").val();
    let routeFetchInvoice = $("#routeFetchInvoice").val()
    let user_id = $("#userId").val();
    ...
    fetchDataInvoiceList(routeFetchInvoice, dataSend, counter);
}
...
function fetchDataInvoice(e, routeFetchInvoice, dataSend, counter) {
...
function getListTypeInvoice(companyID = '') {
    $("#type_voucher").empty();
    $("#type_voucher").append('<option selected="selected" value="">Select Type...</option>');
    if (companyID !== '') {

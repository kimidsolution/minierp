...
$(function () {
    ...
    const routeStoreInvoicePayable = $("#routeStoreInvoicePayable").val();
    ...
    getPartner(companyIdDefault);

    // get list account asset
    getAccountAsset(companyIdDefault);
    ...
    $(".btn-submit").click(function(e){
        ...
        if ($("#myForm").valid()) {
            ...
            var v_posted = (thisButtonIs == "saveDraft") ? 0 : 1;
            ...
            var v_purchase_order = $("input[name=purchase_order]").val();
            let v_account_id_asset = $('#account_id_asset').val();

            var v_nominal_discount = parseCurrency($("input[name=diskonharga]").val());
            var v_nominal_ppn = parseCurrency($("input[name=hargapajak]").val());
            var v_nominal_pph = parseCurrency($("input[name=pph]").val());
            ...
            var dataPost = {
                ...
                nominal_prepaid_income_tax: v_nominal_pph,
...
$(document).on('keyup', '#uangmuka', function(){
    ...
    calculationAllWithoutPajak();
});

// when change harga pajak
$(document).on('keyup', '#harga-pajak', function(){
        ...
        success: function(response) {
            if (response.status) {
                ...
                $.each(response.data,function(key, value) {
                    $("#product-" + counter).append('<option value=' + value.id + '>' + value.text + '</option>');
                });
            }
        },
        error: function(xhr) { // if error occured
...
// to get partner data
function getPartner(companyId) {
    const routeListPartnerVendorBothCompany = $("#routeListPartnerVendorBothCompany").val();
    let dataPost = { company_id: companyId };

    $.ajax({
        url: routeListPartnerVendorBothCompany,
        type: "POST",
        data: dataPost,
        success: function(response) {
            if (response.status) {
                ...
                $.each(response.data,function(key, value) {
                    $("#pelanggan").append('<option value=' + value.id + '>' + value.text + '</option>');
                });
            }
        },
        error: function(xhr) { // if error occured
...
function getAccountAsset(companyId) {
...
function calculationAllWithoutPajak() {
    // perubahan total product
    var sumTotalProduct = 0;
...
function calcPajak() {
    ...
    let valPersen = pajakPPN / 100;
    let totalPpn = (vatEnabled == "1") ? parseInt(parseCurrency(totalBarangJasa)) * valPersen : 0;

    formatInputCurrencyValue('#harga-pajak', totalPpn);
}
...
function calcSubTotal() {
    ...
    var subtotal = (parseInt(parseCurrency(totalBarangJasa)) + parseInt(parseCurrency(hargaPajak))) - parseInt(parseCurrency(hargaPPh));
...
function resetForm()
{
    ...
    cols += '<td><input class="form-control changeqty" type="text" id="qty-0" name="qty[]" value="0" style="text-align: right;"></td>';

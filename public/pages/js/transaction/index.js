$(function () {
    ...
    const transactionType = $("#transactionType").val() === '' ? 'receivable' : 'payable'
    ...
    $("#transaction-datatable tbody").on("click", ".btn-posted", function () {
        let id = $(this).data("id")
        let name = $(this).data("name")
        flagPosted(id, name)
    })
    ...
    function fetchDatatable(isActive = null, companyId = null) {
        ...
        let pageTable = $("#transaction-datatable").DataTable({
            ...
            columns: [
                ...
                { data: "transaction_of" },
                {
                    data: "nominal_balance_debit",
                    "render": function (data) {
                        return 'Rp. ' + checkNegative(formatCurrency(data))
                    }
                },
                {
                    data: "nominal_balance_credit",
                    "render": function (data) {
                        return 'Rp. ' + checkNegative(formatCurrency(data))
                    }
                },
        ...
        $('#transaction-datatable tbody').on('click', 'td .details-control', function () {
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <h5 style="font-size: 0.9rem; margin-top: 35px; text-align:center;">
                                Transaction Detail ${rowData.transaction_of} ${refNumber}
                            </h5>
                        </div>
                        <div class="col-md-6">
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    ...
                    notifAlert("Success, transaction"+ " " + response.data.reference_number +" has been posted", timeSince(new Date(Date.now())))
